...
if(isset($_POST["btn1"])){
	...
	$des=@$_POST['descuento'];
		if($btn=="Nuevo"){
			 ...
			 mysqli_query($conexion,"truncate table temporal"); 
		}
  ...
  include_once('php_conexion2.php');
	...
	mysqli_query($conexion,"insert into temporal (id,codpro,producto,cantidad,precio,importe) values ('','$codpro','$producto','$cantidad','$precio','$importe')");		   
...
include_once('php_conexion2.php');
	...
	if(isset($_POST['txtusu']) && !empty($_POST['txtusu'])){
		$usu=$_POST['txtusu'];
			...
			else{ echo "<script> alert('Error al realizar venta');</script>"; }
			
   mysqli_query($conexion,"insert into venta (id,usuario,cliente,fecha,descuento) values ('$venta','$idusu','$cliente','$fecha','$des')");
   
   		$res=mysqli_query($conexion,"SELECT * FROM temporal");
   	...
   	mysqli_query($conexion,"INSERT INTO detalleventa (id,codpro,producto,cantidad,precio,importe,venta_id) values ('','$codpro','$producto','$cantidad','$precio','$importe','$venta')");

					}
   $sql=mysqli_query($conexion,"SELECT * FROM detalleventa ");  
            while($p=mysqli_fetch_array($sql)){
            $cant=$p['cantidad'];
            $cod=$p['codpro'];  			
   mysqli_query($conexion,"update producto set stock=stock-$cant where id=$cod");
  }
...
echo $usu;?>"/>
   ...
   $pa=mysqli_query($conexion,"SELECT MAX(id)as maximo FROM venta");				
...
include_once('php_conexion2.php');
$sql="SELECT ROUND(SUM(importe),2) as subtotal FROM temporal";
$res=@mysqli_query($conexion,$sql);
if(!$res){
echo " fallo";
}
...
<td><B><input type='text' name='cboaula' id='cboaula' value='<?php 
include_once('php_conexion2.php');
$sql="SELECT ROUND(SUM(importe)*21/100 ,2) as igv FROM temporal";
$res=@mysqli_query($conexion,$sql);
if(!$res){
echo " fallo";
}
...
<td><B><input type='text' name='cboaula' id='cboaula' value='<?php 
include_once('php_conexion2.php');
$sql="SELECT ROUND(SUM(importe)*$des/100 ,2) as igv FROM temporal";
$res=@mysqli_query($conexion,$sql);
if(!$res){
echo " SIN DESCUENTO";
}
...
$sql="SELECT ROUND(SUM(importe)*21/100+SUM(importe)-SUM(importe)*$des/100,2) as total FROM temporal";
$res=@mysqli_query($conexion,$sql);
if(!$res){
echo " fallo";
}
